...
function closeLightbox() {
    ...
    if (lightboxImg) lightboxImg.classList.remove('zoom-active');
}
...
function toggleZoom(e) { e.stopPropagation(); if (lightboxImg) lightboxImg.classList.toggle('zoom-active'); }
...
function handleGesture() {
    if (lightboxImg && lightboxImg.classList.contains('zoom-active')) return;
    if (touchendX < touchstartX - 50) nextImg();
    if (touchendX > touchstartX + 50) prevImg();
}
document.addEventListener('keydown', (e) => {
    if (!lightbox || !lightbox.classList.contains('opacity-100')) return;
    if (e.key === "ArrowRight") nextImg();
    if (e.key === "ArrowLeft") prevImg();
    if (e.key === "Escape") closeLightbox();
});

// Inisialisasi library animasi gulung (AOS)
if (typeof AOS !== 'undefined') {
    AOS.init({
        duration: 800, // Durasi animasi (ms)
        once: true,    // Hanya diputar sekali
        offset: 100    // Jarak pemicu saat di-scroll
    });
}

// Efek Parallax untuk gambar background Hero
const heroBg = document.getElementById('hero-bg');
window.addEventListener('scroll', () => {
    if (heroBg) {
        let scrollPosition = window.scrollY;
        // background bergerak lebih lambat dibanding scroll (0.4x)
